...
void Realm::update_schema(Schema schema, uint64_t version, std::function<void(SharedRealm, SharedRealm)> migration_function)
{
    schema.validate();
    ...
    if (m_config.read_only()) {
        ...
        ObjectStore::verify_no_migration_required(m_schema.compare(schema), false);
    ...
    auto no_changes_required = [&] {
        if (!additive && version < m_schema_version && m_schema_version != ObjectStore::NotVersioned) {
            throw InvalidSchemaVersionException(m_schema_version, version);
        }
        if (version == m_schema_version || additive) {
            if (changes_required.empty()) {
                set_schema(std::move(schema), version);
                return true;
            }
            ObjectStore::verify_no_migration_required(changes_required, additive);
        }
        return false;
    };
    ...
    if (migration_function && !additive) {
        auto wrapper = [&] {
            ...
            migration_function(old_realm, shared_from_this());
        };
